...
# Public function aliases for the sub-package __init__.py re-exports
def build_sequence_features(
    ...
    """
    Convenience wrapper: run the full FeatureEngineer pipeline.

    Args:
        train_df: Raw training DataFrame.
        test_df:  Raw test DataFrame.
        rul_cap:  RUL capping threshold in cycles.

    Returns:
        (train_df, test_df, feature_cols)
    """
    engineer = FeatureEngineer(rul_cap=rul_cap)
    return engineer.process(train_df, test_df)


def select_informative_sensors(train_df: pd.DataFrame, threshold: float = 1e-6) -> List[str]:
    """
    Return list of sensor columns with variance above `threshold`.

    Args:
        train_df:  Raw training DataFrame.
        threshold: Variance floor; columns below are excluded.

    Returns:
        List of informative sensor column names.
    """
    sensor_cols = [c for c in train_df.columns if c.startswith("sensor_")]
    return [c for c in sensor_cols if train_df[c].var() >= threshold]
