...
# Backward-compatible alias: old imports used CMAPSSLoader, not CMAPSSDataLoader
CMAPSSDataLoader = None  # Populated below after class definition
...
# Alias for backward compatibility with src.data.__init__ re-export
CMAPSSDataLoader = CMAPSSLoader


def load_raw_dataset(
    data_dir: Path, name: str = "FD001"
) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    """
    Convenience function: load a single CMAPSS dataset without instantiating the class.

    Args:
        data_dir: Path to the directory containing .txt files.
        name:     Dataset subset identifier (default: "FD001").

    Returns:
        Tuple of (train_df, test_df, rul_df).
    """
    return CMAPSSLoader(data_dir=data_dir).load_single_dataset(name)
